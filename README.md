...
Lesson 8
  1. Разобраться с кодом.
  2. if (messageFromServer.contains("зашел в чат")) { Заменить эту строку на команду /enter, в случае если сервером была прислана такая команда + никнейм, нужно отобразить сообщение у других пользователей "Пользователь nickname зашел в чат"
